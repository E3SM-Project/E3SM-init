#!/bin/bash
 ...
 for year in  `seq -f "%04g" $year1 $year2`; do
     for month in `seq -w 1 12`; do
         histfile=$head.mpaso.hist.am.timeSeriesStatsMonthly.$year-$month-01.nc
         sstfile=$head.mpaso.hist.am.sst.$year-$month-01.nc
         echo ncks -v timeMonthly_avg_activeTracers_temperature -d nVertLevels,0,0 archive/ocn/hist/$histfile SSTDATA/$sstfile
         ncks -v timeMonthly_avg_activeTracers_temperature -d nVertLevels,0,0 $loc$histfile $loc$sstfile
     done
 done
