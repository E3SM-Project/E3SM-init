#!/bin/bash
 ...
 for monssn in  `seq -w 1 12` DJF MAM JJA SON ANN; do
     ...
     dstfile_WE=climo/ocn/SST_${monssn}_${timerange}_climo.WE.nc

    # echo ncks --map $mapfile $sstfile_ren $dstfile_WE
    # ncks --map $mapfile $sstfile_ren $dstfile_WE

     # rotate the hemispheres (WE -> EW)
     dstfile_EW=climo/ocn/SST_${monssn}_${timerange}_climo.nc
    ...
    # ncap2 -O -s 'where(lon < 0) lon=lon+360' $dstfile_EW $dstfile_EW
     ...
     dstfile_WE=climo/ice/${casename}_iceArea_${monssn}_${timerange}_climo.WE.nc
     dstfile_EW=climo/ice/${casename}_iceArea_${monssn}_${timerange}_climo.nc
 ...
 WKDIR=`pwd`
 cd climo/ocn
 ncrcat SST_??_*_climo.nc -o ${casename}_SST_climo_${year1}-${year2}.nc
#  rename Time dimension to time as assumed in later step
 ncrename -d Time,time ${casename}_SST_climo_${year1}-${year2}.nc

 cd $WKDIR
 cd climo/ice
 ncrcat iceArea_??_*_climo.nc -o ${casename}_iceArea_climo_${year1}-${year2}.nc
 ...
 export OUTPUT_SSTICE_FILE=sst_ice_${casename}_climo_${year1}-${year2}.nc
